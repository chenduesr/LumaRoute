...
使用 **Tauri 2 + React + TypeScript + Vite + Tailwind CSS + Radix UI**；业务逻辑由 Rust 实现。当前版本 **0.4.0**。
...
- [下载 LumaRoute 0.4.0](https://github.com/chenduesr/LumaRoute/releases/tag/v0.4.0)：提供 EXE、MSI、源码包与 SHA-256 校验文件。
...
- `src-tauri/target/release/bundle/nsis/LumaRoute_0.4.0_x64-setup.exe`
- `src-tauri/target/release/bundle/msi/LumaRoute_0.4.0_x64_zh-CN.msi`
...
## 0.4.0 Windows TUN 与状态同步

新增无需单独配置系统代理的 Xray 原生 TUN 接管、UAC 提权、Wintun 资源校验、托盘与设置入口，并修复旧查询覆盖新事件导致的界面状态延迟。完整说明见 [0.4.0 发布说明](docs/RELEASE-0.4.0.md)。
